...
console.log('T5 — dual-shop powder is one list')
{
  ...
  ok(sellStep(0.3) === 4, 'ceil not floor: 0.3 needs 4 for a taco')
  ...
  const medBlock = src.slice(src.indexOf("id: 'medicine'"), src.indexOf("id: 'parts'"))
  const partsBlock = src.slice(src.indexOf("id: 'parts'"), src.indexOf("id: 'clothing'"))
  ok(/WARE_WAGON_PRICES\.medicine \/ 2/.test(medBlock), 'town medicine sell is half Matt')
  ok(/WARE_WAGON_PRICES\.parts \/ 2/.test(partsBlock), 'town parts sell is half Matt')
  const medSell = Math.floor((WARE_WAGON_PRICES.medicine / 2) * WARE_WAGON.medicine)
  const partsSell = Math.floor((WARE_WAGON_PRICES.parts / 2) * WARE_WAGON.parts)
  ok(medSell <= WARE_WAGON.medicine * WARE_WAGON_PRICES.medicine, 'Ware chests cannot print at town')
  ok(partsSell <= WARE_WAGON.parts * WARE_WAGON_PRICES.parts, 'Ware axles cannot print at town')
  ok(medSell === 5 && partsSell === 10, '2 chests → 5, 2 axles → 10')
}
