...
// ─── Config ─────────────────────────────────────────────────────────────────
// Micah's tracker — a duplicate of Danna's flat hourly log: just hours worked at
// a flat $20/hr, with a subject + date. No rent / cash / bonus / month machinery
// (that all lives on Mike Fisher's /worker page). Entries are isolated by the
// worker_name key in the shared /api/worker/entries store.
const WORKER_NAME = 'Micah';
const HOURLY_RATE = 20;          // $/hr — flat
...
// ─── Styles (inline — no dependency on site theme; mirrors /worker/danna) ─────
const S = {
...
export default function MicahWorkerPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!fDesc.trim()) return showToast('Please enter a subject for the work');
        ...
        <div style={S.card}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            ...
            <div>
              ...
              <textarea style={S.textarea} placeholder="Subject of the work (e.g. split firewood, fixed fence, cleared brush)..."
