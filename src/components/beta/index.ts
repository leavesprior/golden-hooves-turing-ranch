export { TopDownSceneBeta } from './TopDownSceneBeta'
export {
  WELCOME_OBSERVATION_HOTSPOTS,
  WELCOME_OBSERVATION_SCENE,
  WELCOME_SCENE_ASSETS,
  WELCOME_SCENE_PROPS,
  WELCOME_TERRAIN_TILES,
  WELCOME_WALKWAY_POINTS,
} from './welcomeObservationScene'
export {
  HOUSE_OBSERVATION_HOTSPOTS,
  HOUSE_OBSERVATION_SCENE,
  HOUSE_SCENE_ASSETS,
  HOUSE_SCENE_PROPS,
  HOUSE_TERRAIN_TILES,
  HOUSE_WALKWAY_POINTS,
} from './houseObservationScene'
export type {
  MapPoint,
  ObservationHotspot,
  ObservationSourceLayer,
  SceneAssetMetadata,
  ScenePropData,
  ScenePropKind,
  TerrainKind,
  TerrainTile,
  TopDownSceneBetaProps,
} from './TopDownSceneBeta'
